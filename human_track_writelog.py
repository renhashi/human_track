...
def main():
    ...
    signal.setitimer(signal.ITIMER_REAL, 20, INTERVAL)  #signal.setitimer(signal.ITIMER_REAL, 1回目の実行までの時間, 2回目以降の実行間隔)
    ...
    while cap.isOpened():
		...
		#kari no minus taisaku
                ...
                logger.info("IN:"+str(to_upper))
                logger.info("OUT:"+str(to_lower))
